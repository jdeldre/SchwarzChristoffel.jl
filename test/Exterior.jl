@testset "Exterior map" begin
  ...
  dz, ddz, dddz = derivatives(zeta,m)
  @test dz ≈ ComplexF64[1.0305280030434472 + 0.00444992401906022im
                        1.0069558798697509 - 0.011501133808723474im
                        1.300781073852061 - 0.2666246529806925im]
  @test ddz ≈ ComplexF64[-0.007347853718056296 + 0.010176419159792394im
                         -0.017527300028137238 + 0.002310452313986855im
                          0.7148871959023534 + 0.8455838803110396im]
  @test dddz ≈ ComplexF64[-0.010616344897450836 - 0.013768451421916905im
                          -0.013590848721504067 + 0.021918317742499527im
                          -3.171688554853821 + 2.268096816468463im]
...
@testset "Power series map" begin
  ...
  dz, ddz, dddz = derivatives(zeta,m)
  @test dz ≈ c[1] - c[3]./zeta.^2
  @test ddz ≈ 2.0*c[3]./zeta.^3
  @test dddz ≈ -6.0*c[3]./zeta.^4
